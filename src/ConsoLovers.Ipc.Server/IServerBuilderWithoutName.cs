...
public interface IServerBuilderWithoutName
{
   ...
   /// <param name="name">The name for the server. NOTE: this must be a valid file name, and the client must know this name</param>
   ...
   /// <summary>
   ///    The function that computes the path to the directory where socket files are located. This is only used when a socket file is not specified
   ///    directly.
   /// </summary>
   /// <param name="computeSocketDirectory">The compute socket directory function.</param>
   /// <returns>The server builder</returns>
   IServerBuilder WithSocketDirectory(Func<string> computeSocketDirectory);
